...
public class ShutDownGracefully {
    ...
    public ShutDownGracefully(MultiThreadIoEventLoopGroup boss, MultiThreadIoEventLoopGroup worker, ExecutorService executor) {
        this.bossGroup = boss;
    ...
    public void stop() {
        ...
        log.debug("【TLMQTT】Stopping TL-MQTT server gracefully...");
        try {
            ...
            log.debug("【TLMQTT】TL-MQTT server stopped.");
        } catch (Exception e) {
            log.error("【TLMQTT】Error during shutdown", e);
        }
    }
}
