...
//public class TlConnectHandler1 extends AbstractTlHandler<TlMqttConnectReq>{
//
//    public TlConnectHandler1(SessionService sessionService,
...
//        String clientId = (mqttVersion == MqttVersion.MQTT_5 && req.getPayload().getClientId() == null)?IdUtil.nanoId(12):req.getPayload().getClientId();
...
//            .doOnNext(session -> {
//                //如果客户标识符（ClientID）所代表的客户端已经连接到此服务端，那么向原有的客户端发送一个包含原因码为0x8E（会话被接管）的DISCONNECT报文，
//                if(session != null && mqttVersion == MqttVersion.MQTT_5 && session.getCtx().channel().isActive()){
//                    TlMqttDisconnectReq disconnectReq = TlMqttDisconnectReq.build(MqttErrorCode.CONNECTION_REFUSED_CONNECTION_RATE_EXCEEDED);
//                    session.getCtx().channel().writeAndFlush(disconnectReq).addListener(future -> {session.getCtx().channel().close();
//                    });
//                }
//            })
//            .flatMap(session -> {
...
//                    sessionService.cancelRemoveSession(clientId);
//                    if (cleanSession) {
//                        // 使用 flatMap 确保 clearAll 操作被执行
//                      return sessionService.clearAll(clientId).then(Mono.just(session));
//                    }
//                }
...
//           }
//        }
//        TlMqttConnackAck connackResponse = TlMqttConnackAck.build(sessionPresent, MqttErrorCode.SUCCESS,mqttVersion,req.getPayload().getClientId(),variableHead.getKeepAlive(),mqttConfiguration);
//
...
//                                              publishService.clear(clientId,messageId).subscribe();
//                                               return Mono.empty();
//                                           }
...
//                                       publishService.save(clientId,messageId,publishReq)
//                                            .subscribe(da->{
